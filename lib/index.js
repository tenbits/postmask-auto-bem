				...
				(function(){
					...
					var _node_modules_auto_bem_lib_index = {};
...
exports.Match = Match;
var MatchMapping = /** @class */ (function () {
...
function selectorFlatten(matches, opts) {
    ...
    var flatten = null, type = type_HOST, block = opts && opts.type === 'host' ? null : getBlockByDescendant(matches);
    if (block != null) {
...
function transformTemplate(template, matches, options) {
    ...
    return globals_1.mask.stringify(ast, { indent: minify ? 0 : 4 });
}
...
exports.BemCss = BemCss_1.BemCss;
				...
				;
					...
					if (isObject(_node_modules_auto_bem_lib_index) && isObject(module.exports)) {
						Object.assign(_node_modules_auto_bem_lib_index, module.exports);
						return;
					}
					_node_modules_auto_bem_lib_index = module.exports;
				}());
				// end:source ./templates/ModuleSimplified.js
...
Object.defineProperty(exports, "__esModule", { value: true });
var AutoBem = _node_modules_auto_bem_lib_index;
var _mask = null;
var _io = null;
var _settings = null;
var PLUGIN_NAME = 'postmask-auto-bem';
function initialize(optimizer, settings, mask, io) {
    _mask = mask;
    _io = io;
...
function optimize(root, ctx, next) {
    var bemImports = [], bemDecos = [];
    _mask.TreeWalker.walk(root, function (node) {
        if (Ast.isBemImport(node)) {
            var path = Ast.getFilePathForImport(node, ctx.filename);
            var file = AutoBemUtil.createFile(path, node);
    ...
    _mask.TreeWalker.walk(root, function (node) {
        if (Ast.isBemDeco(node)) {
            try {
                var name = Ast.getBemNameFromDeco(node);
                var import_ = Ast.getBemImportForDeco(node, name);
                var path = Ast.getFilePathForImport(import_, ctx.filename);
                var file = AutoBemUtil.createFile(path, import_);
                var element = Ast.getElementForDeco(node);
    ...
    bemImports.forEach(function (x) { return x.moduleType = null; });
    next();
}
var Ast;
(function (Ast) {
    function isBemImport(node) {
        return node.type === _mask.Dom.COMPONENT && node.tagName === 'import' && node.moduleType === 'bem';
    }
    Ast.isBemImport = isBemImport;
    function getBemNameFromDeco(node) {
        var bemName = /bem\s*\((.+)\)/.exec(node.expression);
        if (bemName) {
            var arg = bemName[1];
            var str = /['"](.+)['"]/.exec(arg);
            if (str == null) {
    ...
    Ast.getBemNameFromDeco = getBemNameFromDeco;
    function getFilePathForImport(node, sourceFilename) {
        return _mask.Module.resolvePath(node, { filename: sourceFilename });
    }
    Ast.getFilePathForImport = getFilePathForImport;
    function getBemImportForDeco(node, name) {
    ...
    Ast.getBemImportForDeco = getBemImportForDeco;
    function isBemDeco(node) {
        return node.type === _mask.Dom.DECORATOR && /bem\s*($|\()/.test(node.expression);
    }
    Ast.isBemDeco = isBemDeco;
    function getElementForDeco(node) {
    ...
    Ast.getElementForDeco = getElementForDeco;
})(Ast || (Ast = {}));
var AutoBemUtil;
(function (AutoBemUtil) {
    function createFile(path, maskImport) {
        var file = new _io.File(path);
        if (file.bemCss != null) {
            return file;
        }
        ...
        var style = file.read();
        var bemCss = new AutoBem.BemCss(style, __assign({ filename: path, salt: maskImport.attr && maskImport.attr.salt }, _settings));
        file.bemCss = bemCss;
        file.content = bemCss.getStyle();
        return file;
    }
    AutoBemUtil.createFile = createFile;
})(AutoBemUtil || (AutoBemUtil = {}));
